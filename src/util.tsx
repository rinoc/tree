...
// Only used when drag, not affect SSR.
export function calcDropPositionCustom<TreeDataType extends BasicDataNode = DataNode>(
  event: React.MouseEvent,
  dragNode: NodeInstance<TreeDataType>,
  targetNode: NodeInstance<TreeDataType>,
  indent: number,
  ...
  const { clientX, clientY } = event;
  const { top, height, bottom } = (event.target as HTMLElement).getBoundingClientRect();
  ...
  // the vertical size on the nodes edges
  // that will trigger a re-order vs drop
  const EDGE_SIZE_PX = 25;

  // let breakpoint1 = top;
  const breakpoint2 = top + EDGE_SIZE_PX;
  const breakpoint3 = bottom - EDGE_SIZE_PX;
  // let breakpoint4 = bottom;

  // let isTopArea = (breakpoint1 < clientY) && (clientY <= breakpoint2);
  const isMidArea = breakpoint2 < clientY && clientY <= breakpoint3;
  // let isBottomArea = (breakpoint3 < clientY) && (clientY <= breakpoint4);
